...
import { AscentV3PoolSwapTest } from '../typechain-types/contracts/test/AscentV3PoolSwapTest'
...
describe('AscentV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('AscentV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as AscentV3PoolSwapTest
          ...
          let tester: AscentV3PoolSwapTest
