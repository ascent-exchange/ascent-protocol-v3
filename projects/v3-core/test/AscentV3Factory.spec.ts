...
describe('AscentV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: AscentV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as AscentV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as AscentV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [AscentV3Factory, AscentV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('AscentV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('AscentV3Pool')
