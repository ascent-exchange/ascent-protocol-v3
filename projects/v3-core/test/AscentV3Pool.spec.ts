...
import { TestAscentV3SwapPay } from '../typechain-types/contracts/test/TestAscentV3SwapPay'
...
import { TestAscentV3ReentrantCallee } from '../typechain-types/contracts/test/TestAscentV3ReentrantCallee'
...
describe('AscentV3Pool', () => {
  ...
  let factory: AscentV3Factory
  let pool: MockTimeAscentV3Pool

  let swapTarget: TestAscentV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeAscentV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestAscentV3ReentrantCallee')
      ).deploy()) as TestAscentV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestAscentV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestAscentV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestAscentV3SwapPay
